/*
 * Please note: I have used hardcoded username and passwords
 * for the API calls as there is no proper validation FR's.
 * 
 * The local variables are for the naff EMA FR implementations.
 */
var app = {
  ...
  receivedEvent: function (id) {
    ...
    // Local storage from API
    GetAllOrderList();
    GetAllOrderItemsList();

    // Initial show map with no client markers
 ...
 * Variable section.
 */
var username; // Used for validation
var password;
  ...
  $.get(
    ...
    function (data) {
      ...
      if (allOrderItemsList.status == 'fail') {
        alert('Get order items: ' + allOrderItemsList.data[0].reason);
      } else return true;
    }
  )
}
  ...
  $.get(
    ...
    function (data) {
      ...
      if (allOrdersList.status == 'fail') {
        alert('Orders list: ' + allOrdersList.data[0].reason);
      }
    }
  )
}
